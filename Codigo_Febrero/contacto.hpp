...
public:
	...
	void modificarHora(int id, string hora)
	{
		_citas[id].hora = hora;
	}

	void modificarFecha(int id, string fecha)
	{
		_citas[id].fecha = fecha;
	}

	void modificarMotivo(int id, string motivo)
	{
		_citas[id].motivo = motivo;
	}

	void removerFecha(int id)
	{
		_citas.erase(_citas.begin() + id);
	}

	void borrarTratamientoEnCurso(string inicio, string fin)
	{
		int j =0;
		for (int i = 0; i < _tratamientos.size(); i++)
		{
			if (_tratamientos[i].fechaInicio == inicio && _tratamientos[i].fechaFinal == fin)
			{
				_tratamientos.erase(_tratamientos.begin() + i);
			}
		}
